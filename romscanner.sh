...
downloadDATs(){
	if [ ! -f $mame_dat ] || [ $p_dats ]; then
		...
		if [ $? -ne 0 ]; then
			echo -e "\033[31mError donwloading MAME DAT file. Try to download manually from [$mame_dat_url] and rename it to [$mame_dat]\e[0m" >&2
			rm -f $mame_dat
			exit 1
		fi
	fi
	if [ ! -f $fba_dat ] || [ $p_dats ]; then
		...
		if [ $? -ne 0 ]; then
			echo -e "\033[31mError donwloading FBA DAT file. Try to download manually from [$fba_dat_url] and rename it to [$fba_dat]\e[0m" >&2
			rm -f $fba_dat
			exit 1
		fi
	fi
